  ...
  return (
    ...
    >
      <div className="mh-stage sticky top-0 h-[100svh] overflow-hidden">
        ...
        <header className="absolute top-0 left-0 right-0 z-30 grid grid-cols-[auto_1fr_auto] items-center gap-3 px-4 py-3 border-b border-[var(--ink)] bg-[var(--paper)]/95 backdrop-blur-sm pointer-events-auto">
          <a className="flex items-center gap-2 font-serif font-semibold text-[15px] text-[var(--ink)]">
            <svg viewBox="0 0 32 32" className="size-5" aria-hidden>
              ...
              <circle cx="16" cy="16" r="3" fill="var(--valve)" />
            </svg>
            Devonel
          </a>
          <span className="font-mono text-[9px] tracking-[0.18em] uppercase text-[var(--ink-2)] text-center truncate">
          ...
          <a className="bg-[var(--ink)] text-[var(--paper)] font-mono text-[10px] tracking-[0.16em] uppercase px-3 py-2 whitespace-nowrap">
            Book →
          </a>
        </header>
        ...
        <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-[var(--paper)] via-[color-mix(in_srgb,var(--paper)_95%,transparent)] to-transparent z-20 pointer-events-none" />
        ...
        <div className="absolute bottom-4 left-3 right-3 z-30 flex items-center gap-3 pointer-events-none">
          <div className="flex gap-1.5">
            {[0, 1, 2, 3].map((i) => (
              <span
                key={i}
                className="mh-dot size-2 rounded-full border border-[var(--ink)] bg-[var(--paper)]"
                data-scene={i}
              />
            ))}
          </div>
          <div className="flex-1 h-[3px] bg-[var(--ink)]/15 relative">
            <span className="mh-progress absolute inset-y-0 left-0 bg-[var(--valve)]" />
          </div>
          <span className="font-mono text-[9px] tracking-[0.18em] uppercase text-[var(--ink-2)] bg-[var(--paper)]/95 backdrop-blur-sm border border-[var(--ink)]/40 px-2 py-1">
...
function MhFigureScene({ index, eyebrow, title, children }: FigSceneProps) {
  return (
    ...
    >
      <div className="font-mono text-[10px] tracking-[0.22em] uppercase text-[var(--ink)] flex items-center gap-3">
        <span className="size-2.5 bg-[var(--valve)]" aria-hidden />
        <span>{eyebrow}</span>
      </div>
